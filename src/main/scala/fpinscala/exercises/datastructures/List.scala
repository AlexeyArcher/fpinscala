...
object List: // `List` companion object. Contains functions for creating and working with lists.
  ...
  def incrementEach(l: List[Int]): List[Int] =
    foldLeft(l, Nil: List[Int], (acc, value) => Cons(value + 1, acc))

  def doubleToString(l: List[Double]): List[String] =
    foldLeft(l, Nil: List[String], (acc, value) => Cons(value.toString(), acc))

  def map[A, B](l: List[A], f: A => B): List[B] =
    foldLeft(l, Nil: List[B], (acc, a) => Cons(f(a), acc))

  def filter[A](as: List[A], f: A => Boolean): List[A] =
    foldLeft(
      as,
      Nil: List[A],
      (tail, head) => if f(head) then Cons(head, tail) else tail
    )
  ...
  def addPairwise(a: List[Int], b: List[Int]): List[Int] =
    (a, b) match
      ...
      case (Cons(head1, tail1), Cons(head2, tail2)) =>
        Cons(head1 + head2, addPairwise(tail1, tail2))
  ...
  def startsWith[A](list: List[A], prefix: List[A]): Boolean =
    (list, prefix) match
      case (_, Nil) => true
      case (Cons(head, tail), Cons(head2, tail2)) if head == head2 =>
        startsWith(tail, tail2)
      case _ => false
